...
import ReleasesFragment from "./ReleasesFragment"
...
const MemoPreferences = React.memo(({ prefs, dispatchPrefs }) => {
					...
					{prefs.desc === "releases" ? (
						...
						<div
							...
							style={{
								...
								// NOTE: className="break-words" does not work as expected.
	...
	const ok = (
		JSONEqual(prev.prefs, next.prefs) &&
		prev.dispatchPrefs === next.dispatchPrefs
	)
	return ok
})
